...
def build_provider_bundle(config: Settings = settings) -> ProviderBundle:
    if config.deployment_profile == "cloud":
        ...
        return ProviderBundle(retrieval=retrieval, generation=generation, reranker=None)
    ...
    return ProviderBundle(retrieval=retrieval, generation=generation, reranker=reranker)
